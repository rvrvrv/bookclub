...
function search(book) {
  ...
  if (!book.trim()) return Materialize.toast('Please enter a book title', 3000, 'error');
  // Then, ensure user entered a new location (to prevent duplicate requests)
  if (book.trim().toLowerCase() === lastSearch) return Materialize.toast('Please enter a new book title', 3000, 'error');
...
function addBook(link) {
  ...
  const apiUrl = `/api/book/${link.getAttribute('id')}/${localStorage.getItem('rv-bookclub-id')}`;
  ajaxFunctions.ajaxRequest('PUT', apiUrl, (data) => {
    ...
    $.post(apiUrl, list[$(link).data('i')])
      ...
      .fail(() => console.error('Could not load data'));
  });
}
